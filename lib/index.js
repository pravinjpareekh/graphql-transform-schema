...
function prepareSubscriptionType(schema, rules) {
    var type = schema.getSubscriptionType();
    if (!type) {
        return;
    }
    var fields = __assign({}, type.getFields());
    ...
    var newSubscriptionType = new graphql_1.GraphQLObjectType({
        name: type.name,
        description: type.description,
        isTypeOf: type.isTypeOf,
        fields: util_1.fieldMapToFieldConfigMapForsubscribe(fields),
        interfaces: type.getInterfaces(),
    });
...
function prepareRules(rules, schema) {
    ...
    var subscriptionFields = schema.getSubscriptionType() ? schema.getSubscriptionType().getFields() : {};
    var allFields = __assign({}, queryFields, mutationFields, subscriptionFields);
    ...
    var _loop_1 = function (fieldName) {
        var matchedRules = _.chain(rules)
            .pickBy(function (val, key) { return val !== false && minimatch(fieldName, key); })
            .values()
            .value();
